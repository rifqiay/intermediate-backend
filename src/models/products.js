...
const searching = (search) => {
  return Pool.query("SELECT * FROM product WHERE name ILIKE $1", [
    `%${search}%`,
  ]);
};
...
const insert = (data) => {
  ...
  return Pool.query(
    `INSERT INTO product(id,name,stock,price,photo,description) VALUES(${id},'${name}',${stock},${price},'${photo}','${description}')`
  );
};
const update = (data) => {
  const { id, name, stock, price, photo, description } = data;
  return Pool.query(
    `UPDATE product SET name='${name}', stock=${stock}, price=${price}, photo='${photo}', description='${description}' WHERE id=${id}`
  );
};
const deleteData = (id) => {
